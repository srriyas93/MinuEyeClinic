...
if (isset($_GET['mode'])) {
    if ($_REQUEST['mode'] == "Login") {
        ...
        $q = "SELECT count(*) as cntUser,status FROM `mec_admin` WHERE `mec_email` = '$username' AND mec_pwd='$md5_password'";
    ...
    elseif ($_REQUEST['mode'] == "resetPasswordContactCheck") {
        ...
        $Q3 = "SELECT count(mob) AS countMobile FROM mec_admin WHERE mob='$reset_pass_mobile'";
    ...
    elseif ($_REQUEST['mode'] == "resetPasswordEmailCheck") {
        ...
        $Q3 = "SELECT count(mec_email) AS countEmail FROM mec_admin WHERE mec_email='$reset_pass_email'";
        $r3 = mysqli_query($con, $Q3);
        if (mysqli_num_rows($r3)>0) {
            $row = mysqli_fetch_array($r3);
    ...
    elseif ($_REQUEST['mode'] == "resetPassword") {
        ...
        $Q3 = "SELECT count(mec_email) AS countEmail,count(mob) AS countMobile FROM mec_admin WHERE mob='$reset_pass_mobile' AND mec_email='$reset_pass_email'";
        $r3 = mysqli_query($con, $Q3);
        if (mysqli_num_rows($r3)>0) {
            $row = mysqli_fetch_array($r3);
            ...
            if ($countemail > 0 && $countMobile > 0) {
                    ...
                    if (mail($to, $subject, $body, $headers)) {
                        ...
                        $Q3 = "UPDATE `mec_admin` SET mec_pwd = '$md5_password', updated_date = CURRENT_TIMESTAMP() WHERE mob='$reset_pass_mobile' AND mec_email='$reset_pass_email'";
